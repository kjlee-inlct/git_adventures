...
for(const contract of ['function inspectCommand(cmd)','function genericMutation(cmd)','function dangerousCommand(cmd)','case "fetch"','case "pull"','case "pushUpstream"','case "pushRejected"','case "push"','case "rebase"','case "merge"','case "stashPush"','case "stashPop"','case "stashConflict"','case "resolveConflict"','case "stashDrop"','case "switchBlocked"','case "clearBlockedSwitch"','case "startRebaseConflict"','case "continueRebase"','case "skipRebase"','case "startMergeConflict"','case "continueMerge"','case "abortOperation"','case "forcePushWithLease"','reset\\s+--hard','clean\\s+-fd','push\\s+--force'])assert.ok(combinedSource.includes(contract),`Simulator contract missing: ${contract}`);
