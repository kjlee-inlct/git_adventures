...
['git fetch origin','remote-inspect'],['git pull','remote-integrate'],['git pull --rebase','remote-rebase'],['git rebase <upstream>','remote-rebase'],['git rebase --continue','conflict-continue'],['git rebase --abort','conflict-abort'],
['git pull --no-rebase','remote-merge'],['git merge <upstream>','remote-merge'],['git merge --abort','conflict-abort'],
['git push','remote-publish'],['git push -u origin <branch>','remote-publish'],['git push --force-with-lease','conditional-rewrite'],
...
const familyTokens=['status','diff','add','restore','switch','commit','revert','fetch','pull','push','stash','rebase','merge'];const covered=new Set(coverage.map(r=>r.command.split(/\s+/)[1]));
