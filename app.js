const translations = {
  en: {
    ...
    hintIntro: "Think about the repository area that must change.", hintExact: "Command shape",
  ...
  ko: {
    ...
    missionNumber: mission => `${mission.track.toUpperCase()} / ${String(mission.number).padStart(2, "0")}`,
    terminalTitle: "practice-repo / terminal",
    hintIntro: "어느 Repository 영역이 바뀌어야 하는지 먼저 생각하세요.", hintExact: "Command 형태",
...
function terminalLine(text, kind = "system") {
  const p = document.createElement("p"); p.className = `terminal-line ${kind}`; p.textContent = text;
...
function resetMissionState() { state = clone(missions[currentMission].initial); stepIndex = 0; resetAttempt(); }
...
function moveFiles(sourceKey, targetKey, names) {
  names.forEach(name => { const source = state[sourceKey]; const index = source.findIndex(file => file.name === name); if (index < 0) return; const [file] = source.splice(index, 1); if (!findFile(state[targetKey], name)) state[targetKey].push(file); });
}
...
function stateFingerprint(value) { return JSON.stringify({ branch: value.branch, working: value.working.map(f => [f.name, f.status]).sort(), staged: value.staged.map(f => [f.name, f.status]).sort(), commits: value.commits }); }
function singleStepTarget(mission) {
  if (mission.steps.length !== 1 || !mission.steps[0].actions?.length) return null;
  const saved = state; state = clone(mission.initial); applyActions(mission.steps[0].actions); const target = clone(state); state = saved; return target;
}
...
function renderState() {
  ...
  $("stateBadge").textContent = t(state.staged.length ? "staged" : state.working.length ? "dirty" : "clean");
  renderFiles("workingTree", state.working); renderFiles("stagingArea", state.staged); $("commitGraph").innerHTML = "";
...
function renderReference() {
  ...
  refs.forEach(([command, description]) => { const div = document.createElement("div"); div.className = "command-chip"; const code = document.createElement("code"); code.textContent = command; const span = document.createElement("span"); span.textContent = description[language]; div.append(code, span); $("commandReference").appendChild(div); });
}
function renderTrackMap() {
  const groups = [...new Set(missions.map(m => m.track))]; $("trackList").innerHTML = "";
  groups.forEach(track => {
    const trackMissions = missions.filter(m => m.track === track); const done = trackMissions.filter(m => completed.has(m.id)).length;
    ...
    const desc = document.createElement("p"); desc.textContent = `Difficulty ${Math.min(...trackMissions.map(m => m.difficulty))}–${Math.max(...trackMissions.map(m => m.difficulty))}`;
...
function commandShape(mission) {
  ...
  const samples = {
    "foundations.inspect.001": "git status",
    "foundations.diff.001": "git diff",
    "foundations.stage.001": "git add <file>",
    "foundations.commit.001": "git commit -m \"<message>\"",
    "workflow.branch.001": "git switch -c <branch>",
    "workflow.atomic.001": stepIndex === 0 ? "git add <related files>" : stepIndex === 1 ? "git diff --staged" : "git commit -m \"<message>\"",
    "recovery.unstage.001": "git restore --staged <file>",
    "recovery.shared.001": "git revert <commit>"
  };
  return samples[mission.id] || "git <command>";
}
function revealHint() {
  const mission = missions[currentMission]; if (attempt.hints >= 3 || completed.has(mission.id)) return;
  ...
  $("missionHint").textContent = text; $("hintDepth").textContent = `${attempt.hints} / 3`; if (attempt.hints >= 3) $("hintButton").disabled = true; updateScores();
}
function renderMission() {
  ...
  $("hintDepth").textContent = "0 / 3"; $("hintButton").disabled = false; $("conceptTitle").textContent = mission.concept.title[language]; $("conceptBody").textContent = mission.concept.body[language];
  $("progressText").textContent = t("progress")(mission.number, missions.length); $("xpText").textContent = `${completed.size * 100} XP`; $("progressBar").style.width = `${(completed.size / missions.length) * 100}%`;
...
function inspectCommand(cmd) {
  if (/^git\s+status$/.test(cmd)) return statusOutput(); if (/^git\s+diff$/.test(cmd)) return diffOutput(state.working, "unstaged");
  if (/^git\s+diff\s+(--staged|--cached)$/.test(cmd)) return diffOutput(state.staged, "staged"); if (/^git\s+log\s+--oneline$/.test(cmd)) return state.commits.join("\n"); return null;
}
function dangerousCommand(cmd) { return /^git\s+(reset\s+--hard|clean\s+-fd|push\s+--force(?:\s|$))/.test(cmd); }
function genericMutation(cmd) {
  if (/^git\s+add\s+\.$/.test(cmd)) { applyAction({ type: "stageAll" }); return { changed: true, message: t("detour") }; }
  const add = cmd.match(/^git\s+add\s+(.+)$/); if (add) { const names = add[1].split(" ").filter(Boolean); const known = names.filter(name => findFile(state.working, name)); if (known.length) { applyAction({ type: "stage", files: known }); return { changed: true, message: language === "ko" ? `${known.join(", ")} Staging 완료.` : `Staged: ${known.join(", ")}` }; } }
  const restore = cmd.match(/^git\s+restore\s+--staged\s+(.+)$/); if (restore) { const names = restore[1].split(" ").filter(Boolean); const known = names.filter(name => findFile(state.staged, name)); if (known.length) { applyAction({ type: "unstage", files: known }); return { changed: true, message: language === "ko" ? `${known.join(", ")} Unstage 완료. Working Tree 변경은 유지됩니다.` : `Unstaged ${known.join(", ")}; working changes are preserved.` }; } }
  return null;
}
...
function completeMission(mission) {
  if (completed.has(mission.id)) return; completed.add(mission.id); terminalLine(t("success"), "success"); $("nextButton").disabled = currentMission === missions.length - 1; $("xpText").textContent = `${completed.size * 100} XP`; $("progressBar").style.width = `${(completed.size / missions.length) * 100}%`; showDebrief(mission); renderTrackMap(); persist();
}
...
function runCommand(raw) {
  const cmd = normalize(raw); if (!cmd) return; const mission = missions[currentMission]; const step = mission.steps[stepIndex]; terminalLine(`$ ${cmd}`, "command");
