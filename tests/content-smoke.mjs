...
if(!Array.isArray(content.missions)||content.missions.length!==23)throw new Error(`Expected 23 missions, got ${content.missions?.length}`);
...
const expected={Foundations:4,'Daily Workflow':7,'Recovery Lab':6,Collaboration:6};for(const [track,count] of Object.entries(expected))if(tracks[track]!==count)throw new Error(`Expected ${count} ${track} missions, got ${tracks[track]||0}`);
