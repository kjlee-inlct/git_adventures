...
['git cherry-pick <sha>','release-backport'],['git cherry-pick --continue','conflict-continue'],['git cherry-pick --abort','conflict-abort'],
...
const familyTokens=['status','diff','add','restore','switch','commit','revert','fetch','pull','push','stash','rebase','merge','cherry-pick'];const covered=new Set(coverage.map(r=>r.command.split(/\s+/)[1]));
