...
if(!Array.isArray(content.missions)||content.missions.length!==18)throw new Error(`Expected 18 missions, got ${content.missions?.length}`);
...
const expected={Foundations:4,'Daily Workflow':7,'Recovery Lab':4,Collaboration:3};for(const [track,count] of Object.entries(expected))if(tracks[track]!==count)throw new Error(`Expected ${count} ${track} missions, got ${tracks[track]||0}`);
