...
if(!Array.isArray(content.missions)||content.missions.length!==13)throw new Error(`Expected 13 missions, got ${content.missions?.length}`);
...
const expected={Foundations:4,'Daily Workflow':6,'Recovery Lab':3};for(const [track,count] of Object.entries(expected))if(tracks[track]!==count)throw new Error(`Expected ${count} ${track} missions, got ${tracks[track]||0}`);
